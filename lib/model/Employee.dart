...
class Employee extends ChangeNotifier {
  ...
  Employee(
      {required this.ID, required this.employeeName, required this.IDCardNumber, required this.type});
  ...
  factory Employee.fromMap(Map<String, dynamic> employee) {
    ...
    return Employee(ID: int.parse(employee['ID']),
  ...
  Future<List<Level>> getLevels() async {
    ...
    levels = List.generate(lsLevels.length, (index) => Level.fromMap(lsLevels[index]));
    return levels;
  }
  ...
  Future<List<Class>> getClasses() async {
    ...
    classes = List.generate(lsClasses.length, (index) => Class.fromMap(lsClasses[index]));
    return classes;
  }
  ...
  Future<List<Student>> getStudents(int classID) async {
    ...
    students = List.generate(lsStudents.length, (index) => Student.fromMap(lsStudents[index]));
    return students;
  }

  static final Employee emptyEmployee = Employee(ID: -1, employeeName: "", IDCardNumber: "", type: EmployeeType.none);

  Future<void> createStudent(int levelID, String parentName, String phoneNumber, String studentName, String birthDate, {required Function(int result) onResult}) async {
      String result = await ClientController.createStudent( levelID,  parentName,  phoneNumber,  studentName,  birthDate);
  ...
  Future<List<TermGrade>> getTermGrade(int studentTermGradeID) async {
      ...
      termGrades = List.generate(lsTermGrades.length, (index) => TermGrade.fromMap(lsTermGrades[index]));
      return termGrades;
  }
  ...
  Future<List<MonthGrade>> getMonthGrade(int termGradeID) async {
    ...
    monthGrades = List.generate(lsMonthGrades.length, (index) => MonthGrade.fromMap(lsMonthGrades[index]));
    return monthGrades;
  }

  void updateMonthGrades(int monthGradeID, double homework, double attendance, double oralTest, double writtenTest) async {
    await ClientController.updateMonthGrades(monthGradeID, homework, attendance, oralTest, writtenTest);
  }
